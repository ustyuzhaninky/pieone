...
class MDCefBrowser(MDWidget):
    ...
    '''
    def __init__(self, start_url='about:blank', **kwargs):
        super(MDCefBrowser, self).__init__(**kwargs)
    ...
    def size_changed(self, *kwargs):
        '''When the height of the MDCefBrowser widget got changed, create the browser
        '''
        if self.starting:
            if self.height != 100:
                self.start_cef()
...
<BrowserLayout>:
    ...
    MDBoxLayout:
        ...
        SearchString:
            id: search_container
            size_hint_y: None
            height: search_string.height
                
            MDTextField:
                id: search_string
                mode: "round"
                icon_left: "language-python"
                hint_text: search_container.hint_text
                text: search_container.text
               
            MDIconButton:
                icon: "search-web"
                pos_hint: {"center_y": .5}
                pos: search_string.width - self.width + dp(8), 0
                theme_text_color: "Hint"
                on_release:
                    browser.change_url(search_container.text)
    MDCefBrowser:
        id: browser
    """

class SearchString(MDRelativeLayout):
    text = StringProperty("about:blank")
...
class BrowserLayout(MDBoxLayout):
    def __init__(self, **kwargs):
        ...
        self.ids.browser.change_url("https://search.brave.com/")

class DemoBrowser(MDApp):
    def build(self):
        self.theme_cls.theme_style = "Dark"
        self.theme_cls.primary_palette = "Orange"
        self.theme_cls.material_style = "M3"
        Builder.load_string(KV)
        self.screen = BrowserLayout()
        return self.screen
