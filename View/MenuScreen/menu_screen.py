    ...
    def swap_images(self, instance, darkmode):
        if darkmode:
            self.ids.simulator_card.source = self.app.asset(
                f"{os.environ['PIEONE_ASSETS']}/"
                "images/menu_screen/simulator-dark.png")
            self.ids.schematic_card.source = self.app.asset(
                f"{os.environ['PIEONE_ASSETS']}/"
                "images/menu_screen/schematic-dark.png")
            self.ids.documentation_card.source = self.app.asset(
                f"{os.environ['PIEONE_ASSETS']}/"
                "images/menu_screen/documentation-dark.png")
            self.ids.about_card.source = self.app.asset(
                f"{os.environ['PIEONE_ASSETS']}/"
                "images/menu_screen/about-dark.png")
        else:
            self.ids.simulator_card.source = self.app.asset(
                f"{os.environ['PIEONE_ASSETS']}/"
                "images/menu_screen/simulator.png")
            self.ids.schematic_card.source = self.app.asset(
                f"{os.environ['PIEONE_ASSETS']}/"
                "images/menu_screen/schematic.png")
            self.ids.documentation_card.source = self.app.asset(
                f"{os.environ['PIEONE_ASSETS']}/"
                "images/menu_screen/documentation.png")
            self.ids.about_card.source = self.app.asset(
                f"{os.environ['PIEONE_ASSETS']}/"
                "images/menu_screen/about.png")
