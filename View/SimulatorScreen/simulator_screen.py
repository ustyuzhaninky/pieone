...
class TouchHoverArea(MDFlatButton, MDTooltip):
    ...
    def open_command_panel_dialog(self, *args):
        if self.screen.emitted_event.text:
            event_text = self.screen.emitted_event.text
        else:
            event_text = self.app.tr._('Unexpected malfunction')
        ...
        command_panel = CommandInterface(
            [ch for i, ch in enumerate(
                EventManager().choises) if i+1 in self.controls],
            [cm for i, cm in enumerate(
                EventManager().commands) if i+1 in self.controls],
            event_text)
...
class CommandInterface(MDBoxLayout):
    ...
    def __init__(
        ...
        self.event_str = event_str if event_str else [
            self.app.tr._('Unexpected malfunction'), '']

        for tx, cm in zip(button_texts, button_commands):
...
class SaveFinishDialogButton(MDFlatButton):
    ...
    def _callback(self):
        try:
            ...
            self.app.log_callback(
                self,
                "FatalError: SaveFinishDialogButton.close_fn "
...
class OkFinishDialogButton(MDRaisedButton):
    ...
    def _callback(self):
        try:
            ...
            self.app.log_callback(
                self,
                "FatalError: OkFinishDialogButton.close_fn "
                "could not be executed")
...
class SkipDecisionDialogButton(MDFlatButton):
    ...
    def _callback(self):
        try:
            ...
            self.app.log_callback(
                self,
                "FatalError: SkipDecisionDialogButton.close_fn "
                "could not be executed")
...
class CancelDecisionDialogButton(MDRaisedButton):
    ...
    def _callback(self):
        try:
            ...
            self.app.log_callback(
                self,
                "FatalError: CancelDecisionDialogButton.close_fn "
                "could not be executed")
...
class SimulatorContent(MDBoxLayout):
    ...
    def solve_event(self, event: Event, node, command):
        ...
        self.ids.image.source = resource_find(
            f"{os.environ['PIEONE_ROOT']}\\assets\\images"
            "\\simulator_screen\\mnemo_bare.png")
    ...
    def __pop_event(self):
        if len(self.scheduled_events) > 0:
            ...
            mean_time = sum([
                event.timing / events_solved for event in self.solved_events if not event.not_attempted])
            total_anwered = sum([
                0 if event.not_attempted else 1 for event in self.solved_events
                ])
            if total_anwered > 0:
                accuracy = sum([
                    1 if event.is_solved else 0 for event in self.solved_events
                    ]) * 100 / total_anwered
            else:
                accuracy = 0.0
            ...
            if not self.finish_dialog:
                ...
                self.finish_dialog_buttons = [
                    SaveFinishDialogButton(
                        close_fn=lambda: go_to_save_dialog()),
                    OkFinishDialogButton(
                        close_fn=lambda:  dismiss_and_stop())]
    ...
    def show_track_record(self) -> None:
        ...
        button = OkTrackRecordDialogButton(
            parent_widget=self.track_record_dialog)
        ...
        self.track_record_dialog = TrackRecordDialog(
            parent_widget=self,
            content_cls=content)
